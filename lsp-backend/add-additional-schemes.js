...
mongoose.connect(process.env.MONGO_URI || 'mongodb://localhost:27017/lsp-bpvp-aceh')
    .then(() => console.log('MongoDB Connected for Additional Data'))
...
const updateSchemes = async () => {
    try {
        ...
        for (const data of schemesToUpdate) {
            console.log(`Processing: ${data.judul}`);
            let skema = await Skema.findOne({ judul: data.judul });

            if (!skema) {
                console.log(`Skema not found, creating new one...`);
                // Assume 'Klaster' type for these as they seem to be klaster based on description
                skema = new Skema({
                    judul: data.judul,
                    jenis: 'Klaster',
